...
// Standard 18-type effectiveness chart, current-gen (unchanged since Gen 6 introduced
// Fairy and removed Steel's old Ghost/Dark resistances). Hardcoded rather than fetched
// from PokeAPI: it's static game data, and hardcoding avoids an 18-way fetch + keeps
// getEffectiveness() synchronous and trivially unit-testable.
//
// Shape: CHART[attackingType][defendingType] = multiplier. Any pair not listed is 1x.
...
/** Product across all of the defender's types (1 type = 1 factor, dual-type = 2 factors). */
export function getEffectiveness(
  attackingType: PokemonTypeName,
  defendingTypes: readonly PokemonTypeName[]
): number {
  let mult = 1;
  for (const t of defendingTypes) mult *= getTypeMultiplier(attackingType, t);
  return mult;
}
